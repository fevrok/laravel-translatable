...
trait Transable
{
     ...
     * @return null
     */
    public function getTrans($attribute, $language = null, $fallback = true)
    {
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('tarjama.locales', [config('tarjama.default')]);
        }

        $response = [];
        foreach ($languages as $language) {
            $response[$language] = $this->getTrans($attribute, $language, $fallback);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('tarjama.default'), false];
        }
        ...
        $default = config('tarjama.default');
    ...
    public function setTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('tarjama.default', 'en');
    ...
    public function deleteTrans($attribute, $locales = null)
    {
        $this->translations()
            ->where('column_name', $attribute)
            ->when(!is_null($locales), function ($query) use ($locales) {
    ...
    public function prepareTranslations(&$request)
    {
        ...
        foreach ($transFields as $field) {
            ...
            $request->merge([$field => $trans[config('tarjama.default', 'en')]]);

            $translations[$field] = $this->setTranslations(
                $field,
                $trans
            );
